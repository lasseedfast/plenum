...
def build() -> str:
    ...
    for table in TABLES:
        ...
        for col in cols:
            ...
            if col["comment"] != PLAIN:
                notes.append(f"{col['column_name']}: {col['comment']}")
        ...
        listing += [f"{pad}\u00b7 {note}" for note in notes]
...
if __name__ == "__main__":
    ...
    if check:
        if current != text:
            raise SystemExit(f"{OUT.relative_to(ROOT)} is out of date. Regenerate it.")
        print(f"{OUT.relative_to(ROOT)} is current.")
    else:
        OUT.write_text(text, encoding="utf-8")
        tokens = len(text) / 3.54
        print(f"wrote {OUT.relative_to(ROOT)} — {len(text):,} chars, ~{tokens:.0f} tokens")
