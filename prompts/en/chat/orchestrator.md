...
1. `search_speeches(query, people, parties, from_year, to_year, limit, return_snippets, person_ids)`
   ...
   - Supports `person_ids=["$person_id_example"]` and `people=["Helena Gellermann"]` to filter
...
- When referring to a politician in text, write Name Lastname (PARTY CODE), e.g. "Jan Riise (MP)".
